...
const char *onion_type_name(int e)
{
	static char invalidbuf[sizeof("INVALID ") + STR_MAX_CHARS(e)];

	switch ((enum onion_type)e) {
...
static u8 *towire_tlv_tlv_payload_amt_to_forward(const tal_t *ctx, const void *vrecord)
{
	...
	towire_tu64(&ptr, r->amt_to_forward->amt_to_forward);
	return ptr;
}
static void fromwire_tlv_tlv_payload_amt_to_forward(const u8 **cursor, size_t *plen, void *vrecord)
{
	...
	r->amt_to_forward = tal(r, struct tlv_tlv_payload_amt_to_forward);
	r->amt_to_forward->amt_to_forward = fromwire_tu64(cursor, plen);
}
...
static u8 *towire_tlv_tlv_payload_outgoing_cltv_value(const tal_t *ctx, const void *vrecord)
{
	...
	ptr = tal_arr(ctx, u8, 0);
	towire_tu32(&ptr, r->outgoing_cltv_value->outgoing_cltv_value);
	return ptr;
}
static void fromwire_tlv_tlv_payload_outgoing_cltv_value(const u8 **cursor, size_t *plen, void *vrecord)
{
	...
	r->outgoing_cltv_value = tal(r, struct tlv_tlv_payload_outgoing_cltv_value);
	r->outgoing_cltv_value->outgoing_cltv_value = fromwire_tu32(cursor, plen);
}
...
static u8 *towire_tlv_tlv_payload_short_channel_id(const tal_t *ctx, const void *vrecord)
{
	...
	ptr = tal_arr(ctx, u8, 0);
	towire_short_channel_id(&ptr, &r->short_channel_id->short_channel_id);
	return ptr;
}
static void fromwire_tlv_tlv_payload_short_channel_id(const u8 **cursor, size_t *plen, void *vrecord)
{
	...
	r->short_channel_id = tal(r, struct tlv_tlv_payload_short_channel_id);
	fromwire_short_channel_id(cursor, plen, &r->short_channel_id->short_channel_id);
}
...
u8 *towire_invalid_onion_payload(const tal_t *ctx, varint type, u16 offset)
{
	u8 *p = tal_arr(ctx, u8, 0);

	towire_u16(&p, WIRE_INVALID_ONION_PAYLOAD);
	towire_varint(&p, type);
	towire_u16(&p, offset);

	return memcheck(p, tal_count(p));
}
bool fromwire_invalid_onion_payload(const void *p, varint *type, u16 *offset)
{
	const u8 *cursor = p;
	size_t plen = tal_count(p);
	...
	if (fromwire_u16(&cursor, &plen) != WIRE_INVALID_ONION_PAYLOAD)
		return false;
 	*type = fromwire_varint(&cursor, &plen);
