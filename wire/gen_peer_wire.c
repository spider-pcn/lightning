...
static u8 *towire_tlv_init_tlvs_networks(const tal_t *ctx, const void *vrecord)
{
	...
	ptr = tal_arr(ctx, u8, 0);
		for (size_t i = 0; i < tal_count(r->networks->chains); i++)
		towire_bitcoin_blkid(&ptr, r->networks->chains + i);
	return ptr;
}
static void fromwire_tlv_init_tlvs_networks(const u8 **cursor, size_t *plen, void *vrecord)
{
	...
	r->networks = tal(r, struct tlv_init_tlvs_networks);
	r->networks->chains = *plen ? tal_arr(r->networks, struct bitcoin_blkid, 0) : NULL;
	for (size_t i = 0; *plen != 0; i++) {
		struct bitcoin_blkid tmp;
		fromwire_bitcoin_blkid(cursor, plen, &tmp);
		tal_arr_expand(&r->networks->chains, tmp);
	}
}
...
static void fromwire_tlv_n1_tlv2(const u8 **cursor, size_t *plen, void *vrecord)
{
	...
	fromwire_short_channel_id(cursor, plen, &r->tlv2->scid);
}
/* N1 MSG: tlv3 */
...
static u8 *towire_tlv_query_channel_range_tlvs_query_option(const tal_t *ctx, const void *vrecord)
{
	...
	ptr = tal_arr(ctx, u8, 0);
	towire_varint(&ptr, r->query_option->query_option_flags);
	return ptr;
}
static void fromwire_tlv_query_channel_range_tlvs_query_option(const u8 **cursor, size_t *plen, void *vrecord)
{
	...
	r->query_option = tal(r, struct tlv_query_channel_range_tlvs_query_option);
	r->query_option->query_option_flags = fromwire_varint(cursor, plen);
}
...
static u8 *towire_tlv_reply_channel_range_tlvs_checksums_tlv(const tal_t *ctx, const void *vrecord)
{
	...
	ptr = tal_arr(ctx, u8, 0);
		for (size_t i = 0; i < tal_count(r->checksums_tlv->checksums); i++)
		towire_channel_update_checksums(&ptr, r->checksums_tlv->checksums + i);
	return ptr;
}
static void fromwire_tlv_reply_channel_range_tlvs_checksums_tlv(const u8 **cursor, size_t *plen, void *vrecord)
{
	...
	r->checksums_tlv = tal(r, struct tlv_reply_channel_range_tlvs_checksums_tlv);
	r->checksums_tlv->checksums = *plen ? tal_arr(r->checksums_tlv, struct channel_update_checksums, 0) : NULL;
	for (size_t i = 0; *plen != 0; i++) {
		struct channel_update_checksums tmp;
		fromwire_channel_update_checksums(cursor, plen, &tmp);
		tal_arr_expand(&r->checksums_tlv->checksums, tmp);
	}
}
...
bool fromwire_reply_short_channel_ids_end(const void *p, struct bitcoin_blkid *chain_hash, u8 *complete)
{
	...
	if (fromwire_u16(&cursor, &plen) != WIRE_REPLY_SHORT_CHANNEL_IDS_END)
		return false;
 	fromwire_bitcoin_blkid(&cursor, &plen, chain_hash);
 	*complete = fromwire_u8(&cursor, &plen);
	return cursor != NULL;
}
