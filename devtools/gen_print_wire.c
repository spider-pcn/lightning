...
void printwire_type_message(const u8 *msg)
{
...
static void printwire_tlv_query_channel_range_tlvs_query_option(const char *fieldname, const u8 **cursor, size_t *plen)
{
	...
	varint query_option_flags = fromwire_varint(cursor, plen);
	
	printwire_varint(tal_fmt(NULL, "%s.query_option_flags", fieldname), &query_option_flags); 
	if (!*cursor) {
		printf("**TRUNCATED**\n");
		return;
	}
...
void printwire_init(const char *fieldname, const u8 *cursor)
{
	...
	printwire_u8_array(tal_fmt(NULL, "%s.localfeatures", fieldname), &cursor, &plen, lflen);
...
void printwire_open_channel(const char *fieldname, const u8 *cursor)
{
	...
	if (!cursor) {
 	...
 	if (plen <= 0)
		return;
	printf("(option_upfront_shutdown_script):");
...
void printwire_channel_reestablish(const char *fieldname, const u8 *cursor)
{
	...
	if (!cursor) {
 	...
 	if (plen <= 0)
		return;
	printf("(option_data_loss_protect,option_static_remotekey):");
...
void printwire_reply_short_channel_ids_end(const char *fieldname, const u8 *cursor)
{
	...
	printwire_u8(tal_fmt(NULL, "%s.complete", fieldname), &complete); 
	if (!cursor) {
		printf("**TRUNCATED**\n");
		return;
	}
...
void printwire_type_tlv_message(const char *tlv_name, const u8 *msg) {
	size_t plen;
	plen = tal_count(msg);
